...
def create_phone_number(num_tuple):
    li = list(num_tuple)
    index_phone = ''.join(str(i) for i in li[:3])
    second_part_phone = ''.join(str(i) for i in li[3:6])
    third_part_phone = ''.join(str(i) for i in li[6:])
    str_phone = '(' + index_phone + ') ' + second_part_phone + '-' + third_part_phone
